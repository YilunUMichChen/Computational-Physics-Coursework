    ...
    for i in range(N):
        for j in range(N):
            if (i > 0 or periodicBC):
                energy += -spins[i][j] * spins[i-1][j] * J
            if (j > 0 or periodicBC):
                energy += -spins[i][j] * spins[i][j-1] * J
            if (i < N - 1 or periodicBC):
                energy += -spins[i][j] * spins[(i+1)%N][j] * J
            if (j < N - 1 or periodicBC):
                energy += -spins[i][j] * spins[i][(j+1)%N] * J
    return J * energy
...
def calculate_and_plot_observables(N, steps, periodicBC=False):
    ...
    for T in temperatures:
        eplot, Mplot = simulateIsing(T, N, steps, periodicBC)
        E_avg = np.mean(eplot)
...
# Plot
calculate_and_plot_observables(N=12, steps=100000, periodicBC=True)